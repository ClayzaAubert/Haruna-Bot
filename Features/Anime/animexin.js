export default {
	...
	haruna: async function (m, { sock, api, text }) {
		...
		try {
			if (action === "stream") {
				const resStream = await api.get("/animexin/stream", { url: queryText });
				...
				const resPopuler = await api.get("/animexin/populer");
				...
				const resLastUpdate = await api.get("/animexin/lastupdate");
